...
- **Ver y editar lo aplicado (nuevo)**: la tarjeta muestra dos contadores por color — "pend." ámbar (`#f57c00`) y "apl." teal (`#2c666e`, nuevo count `estado=Aplicada`). Al abrir el panel (toggle diferido) carga registros de `animal_fumigaciones` (hasta 500, ordenados por fecha DESC) y los lista **agrupados por producto+fecha** mostrando el **nombre del producto, la fecha y la cantidad de animales** aplicados (no una fila por animal) con pill de estado. Cada grupo tiene botones **editar** (mini-formulario inline de producto+fecha que **actualiza todo el lote de una vez** vía `restFetch` PATCH con `producto=eq.&fecha=eq.&empresa_id=eq.`) y **eliminar** (`Snackbar.confirm` + `restFetch` DELETE con el mismo filtro masivo). Botón "refresh" recarga la lista. Al aplicar la fumigación masiva el panel **ya no se cierra**: refresca lista + contadores con `refreshFumigAll()`.
- **Número principal "veces fumigadas" (nuevo)**: el número grande de la tarjeta ya NO cuenta filas programadas. Ahora lee `fecha,producto,estado` (range 0-4999) y cuenta **grupos fecha+producto** (una vez por fumigación masiva aunque sean 19 animales) → `vecesFumigadas` acumulada que nunca vuelve a cero. El chip teal "apl." se eliminó; abajo solo queda el chip ámbar de **pendientes por grupo** (`fumigPendGroups`). `refreshFumigAll()` recalcula ambos (número grande + chip).
