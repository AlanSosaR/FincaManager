...
- El botón de aplicar en `ganado.js` (cableado como `applyBtn.onclick`): valida fecha/producto, calcula `estado` según fecha (`<= hoy` → `Aplicada`, futura → `Programada`, mismo criterio que `detalle_animal.jsx`), consulta `supabase.from('ganado').select('id').neq('estado','Vendido')`, construye una fila por animal `{animal_id, producto, fecha, estado}` e inserta en `animal_fumigaciones` **en chunks de 100** vía QueryBuilder (que agrega `empresa_id` automáticamente). Confirma con `Snackbar.confirm` antes de aplicar. El texto del botón cambia dinámicamente: fecha `<= hoy` → **"Aplicar"**, futura → **"Programar"** (`updateFumigApplyLabel()`, listener `change` en `fumigFecha`). La confirmación indica "Aplicada para hoy" o "Programada para el día {nombre de día}" (p. ej. sábado).
- **Número principal "veces fumigadas"**: el número grande de la tarjeta cuenta **solo grupos fecha+producto con estado `Aplicada`** (`fumigAppliedGroups`) — las `Programada` NO lo incrementan (leído en `renderGanado` y recalculado en `refreshFumigAll()`). Abajo solo queda el chip ámbar "N **pendientes**" (texto completo, verificado) por grupo (`fumigPendGroups`).
- **Aplicar programdas (nuevo)**: en la lista de registros, el botón ✓ de cada grupo `Programada` **solo se puede aplicar el día exacto** (`fecha === hoy`); si la fecha es otro día muestra `Snackbar.show` "No se puede aplicar hoy: la fumigación es para el día {dia_semana} y hoy no corresponde". Hace PATCH masivo `producto=eq.&fecha=eq.&estado=eq.Programada&empresa_id=eq.` → `Aplicada`. El botón **"Aplicar todas"** (header de la lista, visible solo si hay pendientes) aplica con `...?estado=eq.Programada&fecha=eq.{hoy}` — solo las del día actual.
- **Lista por colores**: cada fila `ganado-fumig-row` usa color según fecha — **Aplicada** teal `#2c666e`, **Hoy** verde `#2e7d32` ("Aplicar hoy"), **Atrasada** rojo `#d32f2f`, **Programada (futura)** naranja `#e65100`. Borde izquierdo de 4px + fondo suave por estado. La confirmación de aplicar muestra "día {nombre de semana}".
- El resto de tarjetas (Total, Hembras, Machos, Vacunas, Pesajes, Preñadas, Vendidos) conserva su comportamiento de filtro.
- **Ver y editar lo aplicado**: al abrir el panel carga registros de `animal_fumigaciones` (hasta 500, ordenados por fecha DESC) y los lista **agrupados por producto+fecha** mostrando el **nombre del producto, la fecha y la cantidad de animales** con pill de estado. Cada grupo tiene botones **editar** (mini-formulario inline de producto+fecha que **actualiza todo el lote de una vez** vía `restFetch` PATCH con `producto=eq.&fecha=eq.&empresa_id=eq.`) y **eliminar** (`Snackbar.confirm` + `restFetch` DELETE con el mismo filtro masivo). Botón "refresh" recarga la lista. Al aplicar la fumigación masiva el panel **no se cierra**: refresca lista + contadores con `refreshFumigAll()`.
- `detalle_animal.jsx`: dentro de cada animal, la pestaña Fumigación **solo muestra `Aplicada` y `Programada`** (filtra `Cancelada` en la tabla del mes, el calendario y el día detalle). La tarjeta estadística muestra el **número grande = Aplicadas** ("N aplicadas" junto al nº, con icono check) y en el sub solo el chip de **programadas** si hay (sin la palabra "Aplicadas" duplicada).
