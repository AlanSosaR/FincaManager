...
- **Ver y editar lo aplicado (nuevo)**: la tarjeta muestra dos contadores por color — "pend." ámbar (`#f57c00`) y "apl." teal (`#2c666e`, nuevo count `estado=Aplicada`). Al abrir el panel (toggle diferido) carga los últimos 50 registros (`animal_fumigaciones` ordenados por fecha DESC + nombre del animal vía `ganado`) y los lista por fila con pill de estado. Cada fila tiene botones **editar** (reemplaza la fila por mini-formulario inline de producto+fecha, guarda con `.update().eq('id')` recalculando estado) y **eliminar** (`Snackbar.confirm` + `.delete().eq('id')`). Botón "refresh" recarga la lista. Al aplicar la fumigación masiva el panel **ya no se cierra**: refresca lista + contadores con `refreshFumigAll()`.
